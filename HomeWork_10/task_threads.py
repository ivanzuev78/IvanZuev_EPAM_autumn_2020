...
def get_paginator_pages_sp500(html: Response) -> List:
    ...
    for i in bs_obj.find("div", class_="finando_paging").find_all("a"):
...
def get_all_companies_from_page(html: Response) -> List[Dict]:
    ...
    for company in bsObj.find("main", class_="site-content").find_all(
    ...
    for j, comp in zip(bsObj.find_all("img"), companies):
        if re.match("https://pproxy.markets.businessinsider.com", j["src"]) != "None":
            comp["1_year_growth"] = j.parent.previous_sibling.previous_sibling.contents[
                4
            ].get_text()
            pass
    return companies
...
def get_top_10_most_expensive_companies(
    ...
    top_10 = list(sorted(data, key=lambda x: x["price_in_USD"])[-10:])
    for company in top_10:
        company["price_in_RUB"] = company["price_in_USD"] * usd_to_rub
    return top_10


def get_top_10_min_pe(data: List[Dict]) -> List[Dict]:
    return list(
        filter(
            lambda x: True if "P/E" in x else False,
            sorted(
                filter(lambda x: True if "P/E" in x else False, data),
                key=lambda x: x["P/E"],
            )[:10],
        )
    )
...
def get_top_10_year_growth(data: List[Dict]) -> List[Dict]:
    return list(sorted(data, key=lambda x: float(x["1_year_growth"][:-1]))[-10:])


def get_top_10_profit_52_weeks(data: List[Dict]) -> List[Dict]:
    return list(sorted(data, key=lambda x: x["profit_52_weeks"])[-10:])
